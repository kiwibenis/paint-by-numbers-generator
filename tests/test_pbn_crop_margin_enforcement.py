...
def create_crop_placement() -> ImagePlacementGeometry:
    return ImagePlacementGeometry(
        scale=1.0,
        image_width_mm=230.0,
...
def test_pdf_exporter_clips_pbn_page_to_printer_safe_area() -> None:
    ...
    placement = create_crop_placement()
    ...
    canvas_instance.beginPath.side_effect = (
        clip_path,
        outline_path,
    )
    ...
    clip_path.rect.assert_called_once_with(
        10.0 * mm,
        10.0 * mm,
        190.0 * mm,
        277.0 * mm,
    ...
    canvas_instance.clipPath.assert_called_once_with(
        clip_path,
        stroke=0,
        fill=0,
    )
